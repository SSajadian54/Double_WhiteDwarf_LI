...
const double dx=double(0.0055903875);
const double dy=double(0.0055903875);
...
for(int k=5; k<=50; ++k){    
    ...
    sprintf(filenam0,"./BOmaps4/%c%c%c%c%d.dat",'B','m','a','p',peri);
    ...
    for(int jn=0; jn<n2; ++jn){///Lens_type
        ...
        p.Xsc=p.u;
        p.Ysc=0.0;
        p.xsiz=double(2.0*p.ur+2.5)*p.Rhos;//[RE]
        p.ysiz=double(2.0*p.ur+2.5)*p.Rhos;//[RE]
