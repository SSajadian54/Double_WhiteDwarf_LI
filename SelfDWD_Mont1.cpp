...
const int initial=int(0*500+0);  
const int interval=int(500); 
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
   ...
   sprintf(filenam0,"./files/light/lcF2/files/%c%c%d.dat",'C','_',3);
   distr=fopen(filenam0,"a+"); 
   fclose(distr);              
   
///=============================================================================      
   for(count=int(1+initial); count< int(interval+initial); ++count){  
   do{
...
emax=0.0;   
   ...
   l.inc= RandR(0.0, 0.5)*M_PI/180.0;//rad
   ...
   s.magb=double(-2.5*log10(pow(10.0,-0.4*s.Map)+pow(10.0,-0.4*l.Map))+2.5*log10(s.blend));  
...
if(l.period<=27.4 and s.magb<16.5 and l.period>=1.0){
   Flags=1; 
   sprintf(filenam2,"./files/light/lcF2/files/%c%c%d.dat",'M','_',int(count));
   ...
   fprintf(film,"%.11lf   %.9lf   %.9lf  %.9lf  %.7lf  %.8lf  %.8lf  %.8lf  %d\n",
   ...
   "%.5lf  %.5lf  %.6lf  %.7lf  %.8lf  %.6lf  %.8lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %d\n",//32+6+1
   ...
   l.inc*RA, l.tet*RA, l.ecen, l.period, log10(l.a/(s.Rstar*Rsun)), l.tp, l.ratio, s.cdp, l.limb, 
   s.magG, s.magBP, s.magRP, l.magG, l.magBP, l.magRP, Flags);//32+6+1
   fclose(distr);      
   cout<<"=============================================================="<<endl;
